...
Background <- function(geneNames,
    ...
    sigList <- CheckSignificance(bootList, sigene)
    # gene index
...
ReportTables <- function(geneNames,
    ...
    reportTables <- foreach(i = seq_along(backgroundReport)) %do% {
        ...
        aboveNoise <- (rMatGene > uppper_limit & sigList[[i]][, "lowerBound"] > backgroundUp) |
                      (rMatGene < lower_limit & sigList[[i]][, "upperBound"] < backgroundDown)
